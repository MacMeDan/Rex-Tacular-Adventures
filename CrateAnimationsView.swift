...
class CrateAnimationsView : UIView {
	...
	func setupHierarchy() {
		...
		let animal = UIImageView()
		animal.bounds = CGRect(x:0, y:0, width:80.0, height:80.0)
		var imgBird: UIImage!
		if let imagePath = bundle.pathForResource("Bird.png", ofType:nil) {
			imgBird = UIImage(contentsOfFile:imagePath)
		}else {
			print("** Warning: Could not create image from 'Bird.png'. Please make sure that it is added to the project directly (not in a folder reference).")
		}
		animal.image = imgBird
		animal.contentMode = .Center
		animal.layer.position = CGPoint(x:274.816, y:222.117)
		animal.transform = CGAffineTransformMakeScale(2.38, 2.38)
		fullCrate.addSubview(animal)
		viewsByName["Animal"] = animal
	...
	func addAnimalShakeAnimation() {
		addAnimalShakeAnimationWithBeginTime(0, fillMode: kCAFillModeBoth, removedOnCompletion: false, completion: nil)
	}

	func addAnimalShakeAnimation(completion: ((Bool) -> Void)?) {
		addAnimalShakeAnimationWithBeginTime(0, fillMode: kCAFillModeBoth, removedOnCompletion: false, completion: completion)
	}

	func addAnimalShakeAnimation(removedOnCompletion removedOnCompletion: Bool) {
		addAnimalShakeAnimationWithBeginTime(0, fillMode: removedOnCompletion ? kCAFillModeRemoved : kCAFillModeBoth, removedOnCompletion: removedOnCompletion, completion: nil)
	}

	func addAnimalShakeAnimation(removedOnCompletion removedOnCompletion: Bool, completion: ((Bool) -> Void)?) {
		addAnimalShakeAnimationWithBeginTime(0, fillMode: removedOnCompletion ? kCAFillModeRemoved : kCAFillModeBoth, removedOnCompletion: removedOnCompletion, completion: completion)
	}

	func addAnimalShakeAnimationWithBeginTime(beginTime: CFTimeInterval, fillMode: String, removedOnCompletion: Bool, completion: ((Bool) -> Void)?) {
		...
		if let complete = completion {
			...
			self.layer.addAnimation(representativeAnimation, forKey: "AnimalShake")
			self.animationCompletions[layer.animationForKey("AnimalShake")!] = complete
		}

		let animalRotationAnimation = CAKeyframeAnimation(keyPath: "transform.rotation.z")
		animalRotationAnimation.duration = 2.000
		animalRotationAnimation.values = [0.000 as Float, 0.528 as Float, -0.426 as Float, 0.313 as Float, -0.273 as Float]
		animalRotationAnimation.keyTimes = [0.000 as Float, 0.250 as Float, 0.500 as Float, 0.750 as Float, 1.000 as Float]
		animalRotationAnimation.timingFunctions = [easeInOutTiming, easeInOutTiming, linearTiming, linearTiming]
		animalRotationAnimation.beginTime = beginTime
		animalRotationAnimation.fillMode = fillMode
		animalRotationAnimation.removedOnCompletion = removedOnCompletion
		self.viewsByName["Animal"]?.layer.addAnimation(animalRotationAnimation, forKey:"AnimalShake_Rotation")
	}

	func removeAnimalShakeAnimation() {
		self.layer.removeAnimationForKey("AnimalShake")
		self.viewsByName["Animal"]?.layer.removeAnimationForKey("AnimalShake_Rotation")
	}
